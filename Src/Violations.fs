// http://thatsit.com.au/seo/reports/violations
...
let checkMarkup url =
    async {
        ...
        let arr =
            match status with
            | Abort | Valid -> [||]
            | Invalid (errors, warnings) -> [| constructViolation'' invalidMarkup None invalidMarkupDesc [|errors; warnings|]|]
        return arr
        }
...
let checkInlineCss html =
    async {
        let inlineCssRegex = compileRegex "(?is)<style.*?</style>"
...
let checkInlineJS html =
    async {
        let inlineJSRegex = compileRegex "(?is)<script.*?</script>"
                ...
                | _ ->
                    ...
                    let arr' =
                        match titleLength with
                            ...
                            | _ when titleLength = 0  -> [| constructViolation titleEmpty <| Some index |]
                            | _ -> [||]
    ...
    async {
        ...
        let arr =
            match anchorsCount with
                | _ when anchorsCount > 250 -> [| constructViolation' tooManyLinks None manyLinksDesc anchorsCount |]
                | _ -> [||]
        return arr
        }
